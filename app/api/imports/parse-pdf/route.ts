...
export async function POST(request: Request) {
  try {
    ...
    let text = "";
    for (let page = 1; page <= document.numPages; page++) { const content = await (await document.getPage(page)).getTextContent(); text += `${content.items.map((item) => "str" in item ? item.str : "").join(" ")}\n`; }
    const output = parseChaseStatementText(text); return NextResponse.json(output);
  } catch (error) {
