...
export async function POST(request: Request) {
  ...
  catch (error) { const message = error instanceof z.ZodError ? "The import data was invalid. Please select the CSV again." : error instanceof Error ? error.message : "Unable to save this import."; return NextResponse.json({ error: message }, { status: error instanceof UnauthorizedError ? 401 : 400 }); }
}
export async function GET(request: Request) {
  try { const user = await getCurrentAppUser(); const portfolioId = new URL(request.url).searchParams.get("portfolioId"); const imports = await prisma.importHistory.findMany({ where: { portfolio: { userId: user.id }, ...(portfolioId ? { portfolioId } : {}) }, include: { portfolio: { select: { name: true } } }, orderBy: { importedAt: "desc" } }); return NextResponse.json({ imports: imports.map((entry) => ({ ...entry, portfolioValue: Number(entry.portfolioValue) })) }); }
  catch { return NextResponse.json({ error: "Unable to load import history." }, { status: 500 }); }
}
